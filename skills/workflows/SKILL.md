---
name: mingsi-workflows
description: 🔗 工作流组合 - 定义多种方法串联时的调用顺序与数据传递。适用于新项目启动、复杂问题攻坚、方案迭代优化。
---
...
方法论的威力不在于单独使用某一件工具，而在于**在正确的时机以正确的顺序组合使用。
...
根据地：[具体切入点]
发展路线：
- 第 1 步：[具体行动] → 成功标志：[验证标准]
- 第 2 步：[具体行动] → 成功标志：[验证标准]
- 第 3 步：[具体行动] → 成功标志：[验证标准]
```
- 终止条件：有一条清晰的、从当前能力出发的发展路线。

#### Step 4：持久战略
- 输入：Step 3 的发展路线
- 目标：将发展路线纳入阶段性战略框架，确定当前阶段，设置阶段转换条件
- 输出格式：
```
当前阶段：战略防御期
本阶段核心任务：[Step 3 第 1 步]
...
```
主要矛盾：[根本冲突点]
我的假说：这个冲突导致了[现象]，因为[理由]
验证方式：如果假说正确，应该能观察到[验证标准]
```
- 终止条件：有一个可以验证的假说。
...
```
群众路线（调查研究） → 矛盾分析 → 实践认识论 → 批评与自我批评 → 群众路线（下一轮）
```
...
- **整体循环终止条件**：当群众路线的多源反馈显示"没有新的重要问题"或用户确认满意时，退出循环。
